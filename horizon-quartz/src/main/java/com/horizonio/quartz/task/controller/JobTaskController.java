...
import com.horizonio.quartz.support.RetObj;
import com.horizonio.quartz.support.spring.SpringUtils;
import com.horizonio.quartz.task.domain.ScheduleJob;
import com.horizonio.quartz.task.service.JobTaskService;
...
public class JobTaskController {
    ...
    @ResponseBody
    public ResponseEntity<RetObj> changeJobStatus(HttpServletRequest request, Long jobId, String cmd) {
        ResponseEntity<RetObj> entity;
        ...
        try {
            ...
            retObj.setFlag(true);
            entity = ResponseEntity.ok(retObj);
        } catch (SchedulerException e) {
            log.error(e.getMessage(), e);
            retObj.setMsg("任务状态改变失败！");
            entity = ResponseEntity.badRequest().body(retObj);
        }
        return entity;
    }
